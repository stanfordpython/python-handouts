...
2. Use Python 3.7.2's `pip` to install `virtualenvwrapper` globally.
...
Navigate to the [Python 3.7.2 download page](https://www.python.org/downloads/release/python-372/) in a web browser.
...
```
sredmond$ which python3
/Library/Frameworks/Python.framework/Versions/3.7/bin/python3
sredmond$ python3 --version
Python 3.7.2
```

If you see the output shown above, then Python 3.7.2 was installed correctly on your machine!
...
Run `brew install python3`. This will fetch the latest version of Python (which might not be Python 3.7.2, but that's all we can do).

Then, continue the process above, after the installation of Python 3.7.2.
...
#### `python3 --version` does not show `Python 3.7.2`
In this case, the version of Python 3 your computer wants to run is not the Python 3.7.2 that we just downloaded. Pause and flag down a member of the course staff to help debug, or ask on Piazza. Include as much detail as you can about the problem.
...
sredmond$ python3 --version
Python 3.7.2
sredmond$ python3 -m venv ~/cs41-env
```
...
```
sredmond$ source ~/cs41-env/bin/activate
(cs41-env) sredmond$
```
...
Observe that our command prompt, which previously was `sredmond$`, now is `(cs41-env) sredmond$`. This is one method by which you can see whether a virtual environment is activated.
...
```
sredmond$ source ~/cs41-env/bin/activate
(cs41-env) sredmond$ which python3
/Users/sredmond/cs41-env/bin/python3
(cs41-env) sredmond$ which python
/Users/sredmond/cs41-env/bin/python
(cs41-env) sredmond$ which pip3
/Users/sredmond/cs41-env/bin/pip3
(cs41-env) sredmond$ which pip
/Users/sredmond/cs41-env/bin/pip
```

It looks like all of our python-related commands are now located inside of the virtual environment. That's a good thing! You can confirm this by seeing that `/Users/sredmond/cs41-env/bin:` is the first entry of the string printed by running `echo $PATH`.

Moreover, observe that:

```
(cs41-env) sredmond$ python3 --version
Python 3.7.2
(cs41-env) sredmond$ python --version
Python 3.7.2
(cs41-env) sredmond$ pip3 --version
pip 18.1 from /Users/sredmond/cs41-env/lib/python3.7/site-packages/pip (python 3.7)
(cs41-env) sredmond$ pip --version
pip 18.1 from /Users/sredmond/cs41-env/lib/python3.7/site-packages/pip (python 3.7)
```

The big takeaway is that, inside our active virtual environment, the commands `python` and `pip` now refer to Python 3.7.2 and it's associated package manager.
...
```
(cs41-env) sredmond$ pip install --upgrade pip
Requirement already up-to-date: pip in ./cs41-env/lib/python3.7/site-packages (18.1)
```
...
sredmond$ echo "source ~/cs41-env/bin/activate" >> ~/.bash_profile
sredmond$ tail -n 2 ~/.bash_profile
...
Next, we'll need to find the location of the `virtualenvwrapper.sh` script, which was downloaded by `pip` above. It should be at `/Library/Frameworks/Python.framework/Versions/3.7/bin/virtualenvwrapper.sh`, but let's make sure. Run:

```
$ cat /Library/Frameworks/Python.framework/Versions/3.7/bin/virtualenvwrapper.sh
```

*Note: If you installed Python 3.7.2 using `brew`, then `virtualenvwrapper.sh` is installed in a different location. It is likely installed at `"$(brew --prefix)"/bin/virtualenvwrapper.sh`, so try using this path instead of the above.*
...
New python executable in /Users/sredmond/.virtualenvs/cs41-env/bin/python3
Also creating executable in /Users/sredmond/.virtualenvs/cs41-env/bin/python
Installing setuptools, pip, wheel...
done.
virtualenvwrapper.user_scripts creating /Users/sredmond/.virtualenvs/cs41-env/bin/predeactivate
virtualenvwrapper.user_scripts creating /Users/sredmond/.virtualenvs/cs41-env/bin/postdeactivate
virtualenvwrapper.user_scripts creating /Users/sredmond/.virtualenvs/cs41-env/bin/preactivate
virtualenvwrapper.user_scripts creating /Users/sredmond/.virtualenvs/cs41-env/bin/postactivate
virtualenvwrapper.user_scripts creating /Users/sredmond/.virtualenvs/cs41-env/bin/get_env_details
```
...
/Users/sredmond/.virtualenvs/cs41-env/bin/python
(cs41-env)$ python --version
Python 3.7.2
(cs41-env)$ python -c "print('Hello world.')"
Hello world!
(cs41-env)$ which pip
/Users/sredmond/.virtualenvs/cs41-env/bin/pip
(cs41-env)$ pip --version
pip 18.1 from /Users/sredmond/.virtualenvs/cs41-env/lib/python3.7/site-packages/pip (python 3.7)
```
...
$ echo "source /Library/Frameworks/Python.framework/Versions/3.7/bin/virtualenvwrapper.sh" >> ~/.bash_profile
```
...
If you've successfully made it through these `virtualenvwrapper` instructions, you are almost all ready to go. We will finish by removing up our old non-`virtualenvwrapper` installation.
